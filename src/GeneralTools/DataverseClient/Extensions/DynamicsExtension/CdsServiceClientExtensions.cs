...
namespace Microsoft.PowerPlatform.Dataverse.Client.Dynamics
{
    /// <summary>
    /// Dynamics Application Extensions for ServiceClient
    /// </summary>
    public static class ServiceClientExtensions
    {
        ...
        /// <returns></returns>
        public static Guid CloseQuote(this ServiceClient serviceClient, Guid quoteId, Dictionary<string, DataverseDataTypeWrapper> fieldList, int quoteStatusCode = 3, Guid batchId = default(Guid), bool bypassPluginExecution = false)
        {
            serviceClient._logEntry.ResetLastError();  // Reset Last Error 
            if (serviceClient.DataverseService == null)
            {
                serviceClient._logEntry.Log("Service not initialized", TraceEventType.Error);
                return Guid.Empty;
            }
            ...
            if (quoteStatusCode == 4)
            {
                ...
                if (serviceClient.AddRequestToBatch(batchId, req, "Calling Close Quote as Won", "Request to Close Quote as Won Queued", bypassPluginExecution))
                    return Guid.Empty;

                WinQuoteResponse resp = (WinQuoteResponse)serviceClient.Command_Execute(req, "Closing a Quote in CRM as Won", bypassPluginExecution);
                if (resp != null)
                    return actId;
                else
                    return Guid.Empty;
            ...
            else
            {
                ...
                if (serviceClient.AddRequestToBatch(batchId, req, "Calling Close Quote as Lost", "Request to Close Quote as Lost Queued", bypassPluginExecution))
                    return Guid.Empty;

                CloseQuoteResponse resp = (CloseQuoteResponse)serviceClient.Command_Execute(req, "Closing a Quote in CRM as Lost", bypassPluginExecution);
                if (resp != null)
                    return actId;
                else
                    return Guid.Empty;
        ...
        /// <param name="bypassPluginExecution">Adds the bypass plugin behavior to this request. Note: this will only apply if the caller has the prvBypassPlugins permission to bypass plugins.  If its attempted without the permission the request will fault.</param>
        /// <returns></returns>
        public static Guid CloseOpportunity(this ServiceClient serviceClient, Guid opportunityId, Dictionary<string, DataverseDataTypeWrapper> fieldList, int opportunityStatusCode = 3, Guid batchId = default(Guid), bool bypassPluginExecution = false)
        {
            serviceClient._logEntry.ResetLastError();  // Reset Last Error 
            if (serviceClient.DataverseService == null)
            {
            ...
            if (opportunityStatusCode == 3)
            {
                ...
                if (serviceClient.AddRequestToBatch(batchId, req, "Calling Close Opportunity as Won", "Request to Close Opportunity as Won Queued", bypassPluginExecution))
                    return Guid.Empty;

                WinOpportunityResponse resp = (WinOpportunityResponse)serviceClient.Command_Execute(req, "Closing a Opportunity in CRM as Won", bypassPluginExecution);
                if (resp != null)
                    return actId;
                else
                    return Guid.Empty;
            ...
            else
            {
                ...
                if (serviceClient.AddRequestToBatch(batchId, req, "Calling Close Opportunity as Lost", "Request to Close Opportunity as Lost Queued", bypassPluginExecution))
                    return Guid.Empty;

                LoseOpportunityResponse resp = (LoseOpportunityResponse)serviceClient.Command_Execute(req, "Closing a Opportunity in CRM as Lost", bypassPluginExecution);
                if (resp != null)
                    return actId;
                else
                    return Guid.Empty;
        ...
        public static Guid CloseIncident(this ServiceClient serviceClient, Guid incidentId, Dictionary<string, DataverseDataTypeWrapper> fieldList, int incidentStatusCode = 5, Guid batchId = default(Guid), bool bypassPluginExecution = false)
        {
            serviceClient._logEntry.ResetLastError();  // Reset Last Error 
            if (serviceClient.DataverseService == null)
            {
            ...
            if (serviceClient.AddRequestToBatch(batchId, req4, "Calling Close Incident", "Request to Close Incident Queued", bypassPluginExecution))
                return Guid.Empty;


            CloseIncidentResponse resp4 = (CloseIncidentResponse)serviceClient.Command_Execute(req4, "Closing a incidentId in CRM", bypassPluginExecution);
            if (resp4 != null)
                return actId;
            else
                return Guid.Empty;
        ...
        /// <param name="bypassPluginExecution">Adds the bypass plugin behavior to this request. Note: this will only apply if the caller has the prvBypassPlugins permission to bypass plugins.  If its attempted without the permission the request will fault.</param>
        /// <returns></returns>
        public static Guid CancelSalesOrder(this ServiceClient serviceClient, Guid salesOrderId, Dictionary<string, DataverseDataTypeWrapper> fieldList, int orderStatusCode = 4, Guid batchId = default(Guid), bool bypassPluginExecution = false)
        {
            serviceClient._logEntry.ResetLastError();  // Reset Last Error 
            if (serviceClient.DataverseService == null)
            {
            ...
            if (serviceClient.AddRequestToBatch(batchId, req, "Calling Close Sales Order", "Request to Close Sales Order Queued", bypassPluginExecution))
                return Guid.Empty;


            CancelSalesOrderResponse resp = (CancelSalesOrderResponse)serviceClient.Command_Execute(req, "Closing a Sales Order in CRM as Closed", bypassPluginExecution);
            if (resp != null)
                return actId;
            else
                return Guid.Empty;
        ...
        public static Guid CloseTroubleTicket(this ServiceClient serviceClient, Guid ticketId, string subject, string description, Guid batchId = default(Guid), bool bypassPluginExecution = false)
        {
            ...
            CloseIncidentResponse resp4 = (CloseIncidentResponse)serviceClient.Command_Execute(req4, "Closing a Case in CRM", bypassPluginExecution);
            if (resp4 != null)
