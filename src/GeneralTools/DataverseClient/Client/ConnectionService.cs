...
namespace Microsoft.PowerPlatform.Dataverse.Client
{
	...
	internal sealed class ConnectionService : IConnectionService, IDisposable
	{
		...
		private async Task<IOrganizationService> InitServiceAsync()
		{
			...
			try
			{
				...
				if (!UseExternalConnection)
				{
					...
					user = GetWhoAmIDetails(dvService, guIntialTrackingID);
					dtQueryTimer.Stop();
		...
		private void RefreshInstanceDetails(IOrganizationService dvService, Uri uriOfInstance)
		{
			...
			if (dvService != null)
			{
				...
				Stopwatch dtQueryTimer = new Stopwatch();
				dtQueryTimer.Restart();
				RetrieveCurrentOrganizationResponse resp = (RetrieveCurrentOrganizationResponse)dvService.Execute(new RetrieveCurrentOrganizationRequest() { AccessType = 0, RequestId = guRequestId });
				dtQueryTimer.Stop();
		...
		internal WhoAmIResponse GetWhoAmIDetails(IOrganizationService dvService, Guid trackingID = default(Guid))
		{
			if (dvService != null)
			{
				Stopwatch dtQueryTimer = new Stopwatch();
				...
				try
				{
					...
					var resp = (WhoAmIResponse)dvService.Execute(req);
		...
		*/
		...
		internal static async Task<OrganizationDetailCollection> DiscoverGlobalOrganizationsAsync(Uri discoveryServiceUri, Func<string, Task<string>> tokenProviderFunction , DataverseTraceLogger logSink = null)
		{
			bool isLogEntryCreatedLocaly = false;
			if (logSink == null)
			{
