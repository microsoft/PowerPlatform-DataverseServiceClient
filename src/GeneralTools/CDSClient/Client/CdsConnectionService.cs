		...
		internal static async Task<HttpResponseMessage> ExecuteHttpRequestAsync(string uri, HttpMethod method, string body = default(string), Dictionary<string, List<string>> customHeaders = null, CancellationToken cancellationToken = default(CancellationToken), CdsTraceLogger logSink = null, Guid? requestTrackingId = null, string contentType = default(string), Guid? sessionTrackingId = null)
		{
			...
			try
			{
				using (var _httpRequest = new HttpRequestMessage())
				{
					...
					logSink.Log(string.Format("Begin Sending request to {0} : {2}RequestID={1}", _httpRequest.RequestUri.AbsolutePath, RequestId, sessionTrackingId.HasValue && sessionTrackingId.Value != Guid.Empty ? $" SessionID={sessionTrackingId.Value.ToString()} : " : ""), TraceEventType.Verbose);
					...
					logSink.Log(string.Format("Response for request to WebAPI {0} : StatusCode={1} : {4}RequestID={2} : Duration={3}", _httpRequest.RequestUri.AbsolutePath, _statusCode, RequestId, logDt.Elapsed.ToString(), sessionTrackingId.HasValue && sessionTrackingId.Value != Guid.Empty ? $" SessionID={sessionTrackingId.Value.ToString()} : " : ""));
					...
					if (!_httpResponse.IsSuccessStatusCode)
					{
						...
						logSink.Log(string.Format("Failure Response for request to WebAPI {0} : StatusCode={1} : {4}RequestID={3} : {2}", _httpRequest.RequestUri.AbsolutePath, _statusCode, _responseContent, RequestId, sessionTrackingId.HasValue && sessionTrackingId.Value != Guid.Empty ? $" SessionID={sessionTrackingId.Value.ToString()} : " : ""), TraceEventType.Error);
					...
					// Removing for now as will replace with request specfiice decoder.
					...
					//	try
					//	{
					//		_result.Body = _responseContent;// SafeJsonConvert.DeserializeObject<IList<Region>>(_responseContent, DeserializationSettings);
					//	}
