...
namespace Microsoft.PowerPlatform.Cds.Client
{
    ...
    public sealed class CdsServiceClient : IOrganizationService, IDisposable
    {
        ...
        public OrganizationResponse ExecuteCdsOrganizationRequest(OrganizationRequest req, string logMessageTag = "User Defined")
        {
            if (req != null)
        ...
        private void RetryRequest(OrganizationRequest req, Guid requestTrackingId, TimeSpan LockWait, Stopwatch logDt, Exception ex, string errorStringCheck, ref int retryCount, bool isThrottled)
        {
            retryCount++;
            LogFailure(req, requestTrackingId, LockWait, logDt, ex, errorStringCheck);
        ...
        private void LogFailure(OrganizationRequest req, Guid requestTrackingId, TimeSpan LockWait, Stopwatch logDt, Exception ex, string errorStringCheck, bool isTerminalFailure = false)
        {
        ...
        private void LogRetry(int retryCount, OrganizationRequest req, bool isTerminalFailure = false, bool isThrottled = false)
        {
            if (retryCount == 0)
            {
                logEntry.Log(string.Format(CultureInfo.InvariantCulture, "No retry attempted for Command {0}", req.RequestName), TraceEventType.Verbose);
            }
            else if (isTerminalFailure == true)
            {
                logEntry.Log(string.Format(CultureInfo.InvariantCulture, "Retry Completed at {0} for Command {1}", $"Retry No={retryCount}", req.RequestName), TraceEventType.Verbose);
            }
            else
            {
                logEntry.Log(string.Format(CultureInfo.InvariantCulture, "{0} for Command {1}", $"Retry No={retryCount} Retry=Started IsThrottle={isThrottled} Delay={_retryPauseTimeRunning.ToString()}", req.RequestName), TraceEventType.Verbose);
            }
        }
        ...
        /// <returns></returns>
        internal async Task<HttpResponseMessage> CdsCommand_WebExecute(string queryString, string body, HttpMethod method, Dictionary<string, List<string>> customHeaders, string contentType, string errorStringCheck)
        {
            Guid requestTrackingId = Guid.NewGuid();
            ...
            DateTime logDt = DateTime.UtcNow;
            ...
            {
                logEntry.Log(string.Format("Invalid URI formed for request - {0}", string.Format("{0}{1}", ConnectUri, queryString)), TraceEventType.Error);
                return null;
            }
            ...
            try
            {
                var a = await CdsConnectionService.ExecuteHttpRequestAsync(TargetUri.ToString(), method, body: body, customHeaders: customHeaders, logSink: logEntry, contentType: contentType, requestTrackingId: requestTrackingId, sessionTrackingId: SessionTrackingId.HasValue ? SessionTrackingId.Value : Guid.Empty).ConfigureAwait(false);
                return a;
            }
            //catch (HttpOperationException opEx )
            //{
            //    logEntry.Log(string.Format(CultureInfo.InvariantCulture, "Failed to Execute Command - {0} : RequestID={1}", queryString, requestTrackingId.ToString()), TraceEventType.Verbose);
            //}
            catch (System.Exception ex)
            {
                logEntry.Log(string.Format(CultureInfo.InvariantCulture, "Failed to Execute Command - {0} : {2}RequestID={1}", queryString, requestTrackingId.ToString(), SessionTrackingId.HasValue && SessionTrackingId.Value != Guid.Empty ? $"SessionID={SessionTrackingId.Value.ToString()} : " : ""), TraceEventType.Verbose);
        ...
        public Guid Create(Entity entity)
        {
            ...
            CreateResponse resp = (CreateResponse)ExecuteCdsOrganizationRequest(new CreateRequest() { Target = entity }, "Create To CDS via IOrganizationService");
            if (resp == null)
                throw LastCdsException;
        ...
        {
            // Relay to Update request. 
            UpdateResponse resp = (UpdateResponse)ExecuteCdsOrganizationRequest(new UpdateRequest() { Target = entity }, "UpdateRequest To CDS via IOrganizationService");
            if (resp == null)
                throw LastCdsException;
        }
