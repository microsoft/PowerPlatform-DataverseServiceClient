    ...
    public sealed class CdsServiceClient : IOrganizationService, IDisposable
    {
        ...
        public CdsServiceClient(Uri instanceUrl, Func<string, Task<string>> tokenProviderFunction, bool useUniqueInstance = true)
        {
            GetAccessToken = tokenProviderFunction ??
                throw new CdsClientConnectionException("tokenProviderFunction required for this constructor", new ArgumentNullException("tokenProviderFunction"));  // Set the function pointer or access. 
            ...
            )
        {
            ...
            else
            {
                if (requestedAuthType == AuthenticationType.ExternalTokenManagement)
                {
                    ...
                    else
                    {
                        ...
                        throw new CdsClientConnectionException("tokenProviderFunction required for ExternalTokenManagement Auth type, You must use the appropriate constructor for this auth type.", new ArgumentNullException("tokenProviderFunction"));
                    }
                }
                else
                {
            ...
            if (CdsConnectionSvc != null)
            {
                ...
                {
                    throw new Utils.CdsClientConnectionException("Failed to connect to Common Data Service", ex);
                }
            }
        }
        ...
        public Guid CreateNewRecord(string entityName, Dictionary<string, CdsDataTypeWrapper> valueArray, string applyToSolution = "", bool enabledDuplicateDetection = false, Guid batchId = default(Guid), bool bypassPluginExecution = false)
        {
            ...
            if (!string.IsNullOrWhiteSpace(applyToSolution))
                createReq.Parameters.Add(Utilities.CDSRequestHeaders.SOLUTIONUNIQUENAME, applyToSolution);
        ...
        public bool UpdateEntity(string entityName, string keyFieldName, Guid id, Dictionary<string, CdsDataTypeWrapper> fieldList, string applyToSolution = "", bool enabledDuplicateDetection = false, Guid batchId = default(Guid), bool bypassPluginExecution = false)
        {
            logEntry.ResetLastError();  // Reset Last Error 
            ...
            UpdateResponse resp = (UpdateResponse)ExecuteCdsOrganizationRequestImpl(req, string.Format(CultureInfo.InvariantCulture, "Updating {0} : {1}", entityName, id.ToString()), useWebAPI: true, bypassPluginExecution:bypassPluginExecution);
            if (resp == null)
                return false;
            else
                return true;
        ...
        public bool DeleteEntity(string entityType, Guid entityId, Guid batchId = default(Guid), bool bypassPluginExecution = false)
        {
            ...
            DeleteResponse resp = (DeleteResponse)ExecuteCdsOrganizationRequestImpl(req, string.Format(CultureInfo.InvariantCulture, "Trying to Delete. Entity = {0}, ID = {1}", entityType, entityId), useWebAPI: true , bypassPluginExecution: bypassPluginExecution);
            if (resp != null)
            {
            ...
            )
        {
            ...
            retrieved = (RetrieveMultipleResponse)CdsCommand_Execute(retrieve, "GetEntityDataBySearchParms", bypassPluginExecution);
            if (retrieved != null)
            {
        ...
        public EntityCollection GetEntityDataByFetchSearchEC(string fetchXml, Guid batchId = default(Guid), bool bypassPluginExecution = false)
        {
            ...
            retrieved = (RetrieveMultipleResponse)CdsCommand_Execute(req, "GetEntityDataByFetchSearch - Direct", bypassPluginExecution);
            if (retrieved != null)
            {
            ...
            bool bypassPluginExecution = false)
        {
            ...
            retrieved = (RetrieveMultipleResponse)CdsCommand_Execute(retrieve, "GetEntityDataByFetchSearch", bypassPluginExecution);
            if (retrieved != null)
            {
                outPageCookie = retrieved.EntityCollection.PagingCookie;
                isMoreRecords = retrieved.EntityCollection.MoreRecords;
            ...
            bool bypassPluginExecution = false)
        {
            ...
            retrieved = (RetrieveMultipleResponse)CdsCommand_Execute(req, "Search On Linked Data", bypassPluginExecution);
        ...
        public Dictionary<string, object> GetEntityDataById(string searchEntity, Guid entityId, List<string> fieldList, Guid batchId = default(Guid), bool bypassPluginExecution = false)
        {
            ...
            RetrieveResponse resp = (RetrieveResponse)CdsCommand_Execute(req, string.Format(CultureInfo.InvariantCulture, "Trying to Read a Record. Entity = {0} , ID = {1}", searchEntity, entityId.ToString()), bypassPluginExecution);
            if (resp == null)
                return null;
		...
		/// <param name="bypassPluginExecution">Adds the bypass plugin behavior to this request. Note: this will only apply if the caller has the prvBypassPlugins permission to bypass plugins.  If its attempted without the permission the request will fault.</param>
        /// <returns></returns>
        public Guid CreateAnnotation(string targetEntityTypeName, Guid targetEntityId, Dictionary<string, CdsDataTypeWrapper> fieldList, Guid batchId = default(Guid), bool bypassPluginExecution = false)
        {
            logEntry.ResetLastError();  // Reset Last Error 
                ...
                if (!string.IsNullOrWhiteSpace(creatingUserId))
                {
                    ...
                    if (userId != Guid.Empty)
                    {
                        ...
                        if (AddRequestToBatch(batchId, arRequest, string.Format(CultureInfo.InvariantCulture, "Trying to Assign a Record. Entity = {0} , ID = {1}", newAction.LogicalName, principal.LogicalName),
                            ...
                            return Guid.Empty;
                        CdsCommand_Execute(arRequest, "Assign Activity", bypassPluginExecution);
                    }
                }
            }
            catch (Exception exp)
        ...
        private bool UpdateStateStatusForEntity(string entName, Guid entId, string newState, string newStatus, int newStateid = -1, int newStatusid = -1, Guid batchId = default(Guid), bool bypassPluginExecution = false)
        {
            ...
            if (AddRequestToBatch(batchId, req, string.Format(CultureInfo.InvariantCulture, "Setting Activity State in CDS... {0}", entName), string.Format(CultureInfo.InvariantCulture, "Request for SetState on {0} queued", entName),bypassPluginExecution))
                return false;

            SetStateResponse resp = (SetStateResponse)CdsCommand_Execute(req, string.Format(CultureInfo.InvariantCulture, "Setting Activity State in CDS... {0}", entName), bypassPluginExecution);
            if (resp != null)
                return true;
            else
                return false;
            ...
            )
        {
            ...
            RollupResponse resp = (RollupResponse)CdsCommand_Execute(req, string.Format(CultureInfo.InvariantCulture, "Locating {0} by ID in CDS GetActivitesBy", searchEntity), bypassPluginExecution);
            if (resp == null)
                return null;
                ...
                if (rslts.Count > 0)
                {
                    foreach (Dictionary<string, object> row in rslts.Values)
                    {
                        ...
                        if (guWorkflowID != Guid.Empty)
                        {
                            ...
                            ExecuteWorkflowResponse wfResponse = (ExecuteWorkflowResponse)CdsCommand_Execute(wfRequest, string.Format(CultureInfo.InvariantCulture, "Executing workflow ({0}) on ID {1}", workflowName, id), bypassPluginExecution);
                            if (wfResponse != null)
        ...
        public bool CreateEntityAssociation(string entityName1, Guid entity1Id, string entityName2, Guid entity2Id, string relationshipName, Guid batchId = default(Guid), bool bypassPluginExecution = false)
        {
            ...
            AssociateEntitiesResponse resp = (AssociateEntitiesResponse)CdsCommand_Execute(req, "Executing CreateEntityAssociation", bypassPluginExecution);
            if (resp != null)
                return true;
        ...
        public bool CreateMultiEntityAssociation(string targetEntity, Guid targetEntity1Id, string sourceEntityName, List<Guid> sourceEntitieIds, string relationshipName, Guid batchId = default(Guid), bool bypassPluginExecution = false, bool isReflexiveRelationship = false)
        {
            ...
            AssociateResponse resp = (AssociateResponse)CdsCommand_Execute(req, "Executing CreateMultiEntityAssociation", bypassPluginExecution);
            if (resp != null)
                return true;
        ...
        public bool DeleteEntityAssociation(string entityName1, Guid entity1Id, string entityName2, Guid entity2Id, string relationshipName, Guid batchId = default(Guid), bool bypassPluginExecution = false)
        {
            ...
            DisassociateEntitiesResponse resp = (DisassociateEntitiesResponse)CdsCommand_Execute(req, "Executing DeleteEntityAssociation", bypassPluginExecution);
            if (resp != null)
                return true;
        ...
        public bool AssignEntityToUser(Guid userId, string entityName, Guid entityId, Guid batchId = default(Guid), bool bypassPluginExecution = false)
        {
            ...
            AssignResponse arResp = (AssignResponse)CdsCommand_Execute(assignRequest, "Assigning Entity to User", bypassPluginExecution);
            if (arResp != null)
                return true;
            ...
            if (rslts != null)
                if (rslts.Count > 0)
                {
                    ...
                    if (guQueueID != Guid.Empty)
                    {
                        ...
                        AddToQueueResponse resp = (AddToQueueResponse)CdsCommand_Execute(req, string.Format(CultureInfo.InvariantCulture, "Adding a item to queue {0} in CDS", queueName), bypassPluginExecution);
                        if (resp != null)
                            return true;
                        else
                            return false;
        ...
        public bool SendSingleEmail(Guid emailid, string token, Guid batchId = default(Guid), bool bypassPluginExecution = false)
        {
            ...
            SendEmailResponse sendresp = (SendEmailResponse)CdsCommand_Execute(req, string.Format(CultureInfo.InvariantCulture, "Sending email ({0}) from CDS", emailid), bypassPluginExecution);
            if (sendresp != null)
                return true;
            else
                return false;
        ...
        {
            return ExecuteCdsOrganizationRequestImpl(req, logMessageTag, useWebAPI, false);
        }

        private OrganizationResponse ExecuteCdsOrganizationRequestImpl(OrganizationRequest req, string logMessageTag = "User Defined", bool useWebAPI = false, bool bypassPluginExecution = false)
        {
            if (req != null)
            {
                ...
                if (!useWebAPI)
                {
                    return CdsCommand_Execute(req, logMessageTag, bypassPluginExecution);
                }
                else
                {
                    // use Web API. 
        ...
        private OrganizationResponse CdsCommand_WebAPIProcess_Execute(OrganizationRequest req, string logMessageTag , bool bypassPluginExecution)
        {
            if (Utilities.IsRequestValidForTranslationToWebAPI(req)) // THIS WILL GET REMOVED AT SOME POINT, TEMP FOR TRANSTION  //TODO:REMOVE ON COMPELTE
            {
                ...
                if (cReq != null)
                {
                    ...
                    var EntityData = metadataUtlity.GetEntityMetadata(EntityFilters.Relationships, cReq.LogicalName);
                    if (EntityData == null)
                    {
                    ...
                    if (req.Parameters.ContainsKey(Utilities.CDSRequestHeaders.BYPASSCUSTOMPLUGINEXECUTION))
                    {
                        if (req.Parameters[Utilities.CDSRequestHeaders.BYPASSCUSTOMPLUGINEXECUTION].GetType() == typeof(bool) &&
                             (bool)req.Parameters[Utilities.CDSRequestHeaders.BYPASSCUSTOMPLUGINEXECUTION])
                        {
        ...
        {
            AssociateResponse resp = (AssociateResponse)ExecuteCdsOrganizationRequestImpl(new AssociateRequest()
            {
        ...
        {
            DeleteResponse resp = (DeleteResponse)ExecuteCdsOrganizationRequestImpl(
                new DeleteRequest()
                {
        ...
        {
            DisassociateResponse resp = (DisassociateResponse)ExecuteCdsOrganizationRequestImpl(new DisassociateRequest()
            {
                Target = new EntityReference(entityName, entityId),
                Relationship = relationship,
                RelatedEntities = relatedEntities
        ...
        {
            OrganizationResponse resp = ExecuteCdsOrganizationRequestImpl(request, string.Format("Execute ({0}) request to CDS from IOrganizationService", request.RequestName) , useWebAPI:true);
            if (resp == null)
                throw LastCdsException;
            return resp;
        }
        ...
        {
            RetrieveResponse resp = (RetrieveResponse)ExecuteCdsOrganizationRequestImpl(
                new RetrieveRequest()
                {
                    ColumnSet = columnSet,
                    Target = new EntityReference(entityName, id)
        ...
        {
            RetrieveMultipleResponse resp = (RetrieveMultipleResponse)ExecuteCdsOrganizationRequestImpl(new RetrieveMultipleRequest() { Query = query }, "RetrieveMultiple to CDS via IOrganizationService");
            if (resp == null)
                throw LastCdsException;
